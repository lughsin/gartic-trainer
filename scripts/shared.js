...
export const normalizeWord = (value) => value
    ...
    .toLowerCase()
    .trim();
export const slugify = (value) => normalizeWord(value).replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
const verbPatterns = ['ando', 'endo', 'indo', 'ar', 'er', 'ir'];
export function classifyDifficulty(word, category) {
    const n = normalizeWord(word);
    if (category === 'verbos')
        return 'hard';
    if (category === 'desenho animado')
        return 'medium';
    if (/amor|saudade|justica|tempo/.test(n))
        return 'abstract';
    if (/manga|vela|banco/.test(n))
        return 'ambiguous';
    if (verbPatterns.some((p) => n.endsWith(p)))
        return 'hard';
    return n.length <= 5 ? 'easy' : 'medium';
}
export function generateRecipe(word, category) {
    const n = normalizeWord(word);
    const generic = {
